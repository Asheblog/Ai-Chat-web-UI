...
- **Auto Read（自动读取）**：搜索完成后自动触发网页读取，读取搜索结果中的网页。在 UI 中归入其所属的搜索合并组内部
...
- **Brand Theme（品牌主题）**：管理员可覆盖的品牌视觉变量子集（主色、主色前景、纸感背景、表面、前景、次要前景）；未配置时使用 Claude 默认主题；不可覆盖字号、间距、圆角等结构 Token

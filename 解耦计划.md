...
### 阶段三：收尾与优化期（约 1–2 周）
- [x] 彻底迁移 `lib/api.ts`，仅保留 Axios 工厂，其余 API 均移至 `features` 目录。
- [x] 清理遗留的旧引用（直接使用 `apiClient`、legacy store 字段），确保导入路径统一。
- [x] 覆盖剩余大文件（如 animations 以外的组件、hooks），按照行数目标继续拆分。（本轮完成 Task Trace 控件拆分，后续继续推进行数清单）
- [x] 在 `README.md` 与内部文档中更新新的目录结构、脚本运行方式。
- [x] 观察 `lint:lines` 结果：若平均行数仍 >130，列出未达标文件并制定后续计划。

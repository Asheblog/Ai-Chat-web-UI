...
- [ ] **视图共享逻辑（P0 / 第 1.5–2 周）**  
  ...
  - [ ] 更新 `hooks/use-chat-composer.ts` 与 `components/chat/chat-composer-panel.tsx`，统一引用上述 hook；为聊天主流程跑一次 `vitest --runInBand`。  
...
- [x] 消息气泡：拆分 `MessageHeader/MessageBodyContent/ReasoningSection/ToolTimeline/ShareBadge`，引入 memo selector，确保单个文件 <250 行。  
  - [ ] Settings：建立 `features/settings/pages/{system-general,system-models}`，迁移逻辑并补充快照测试，验证跨平台（Windows/Linux）路径引用无误。
- [ ] **工具与动画模块化（与上一步可并行）**  
  ...
  - [ ] 为工具事件补充 `vitest` 单测（start/result/error）以及可视化组件 story 校验。  
  - [ ] 将 `lib/animations.ts` 拆分为 `lib/animations/{page,chat,sidebar}.ts`，并提供顶层 `index.ts` 统一导出；同步更新引用（Next layout、sidebar 等）。  
  - [ ] 检查 Windows bat / Linux sh 构建脚本是否仍引用旧动画路径，必要时同步。
- [ ] **行数与质量闸**  
  - [ ] 任务收尾前运行 `pnpm lint:lines`，剔除已完成模块的 whitelist；整理剩余超限清单并登记在 issue/PR 描述。  
  - [ ] 对新建 slice/hook/组件补充 `README` 或内联使用说明，确保后续成员可直接接手。

...
import { CheckCircle2, Edit3, Link2, Loader2, ShieldAlert, Trash2 } from "lucide-react"
import type { SystemConnectionGroup } from "@/services/system-connections"
import { deriveChannelName, formatDate } from "@/lib/utils"
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-muted-foreground">
            列表按端点组展示，每个组下面直接看到 Key 标签、密钥摘要和模型范围。
          </p>
        </div>
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline" className="rounded-full px-3 py-1">
                      {group.apiKeys.length} Keys
                    </Badge>
                  </div>
                  <div>
                    <div className="text-base font-semibold">{group.baseUrl}</div>
                    <div className="mt-1 text-sm text-muted-foreground">
                      更新于 {formatDate(group.updatedAt)}，创建于 {formatDate(group.createdAt)}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span className="rounded-full bg-muted px-3 py-1">Auth: {group.authType}</span>
                    {group.prefixId ? <span className="rounded-full bg-muted px-3 py-1">Prefix: {group.prefixId}</span> : null}
                    <span className="rounded-full bg-muted px-3 py-1">Type: {group.connectionType}</span>
                    {group.tags.length > 0 ? (
                      <span className="rounded-full bg-muted px-3 py-1">
                        Tags: {group.tags.map((item) => item.name).join(", ")}
                      </span>
                    ) : null}
                  </div>
                </div>

...
import { SettingsTabs, type SettingsTabDef } from '@/components/settings/components/settings-tabs'
...
const AUDIT_TABS: SettingsTabDef[] = [
  {
    key: 'skill-audit',
    label: 'Skill 审计',
    icon: ShieldCheck,
...
export function SystemSkillAuditsPage() {
  ...
  return (
    <div className="min-w-0 space-y-4">
      <SettingsTabs
        tabs={AUDIT_TABS}
        defaultTab="skill-audit"
        titleOf={(tab) => `${tab.label}日志`}
        renderContent={(activeKey) => (
          <>
            {activeKey === 'skill-audit' && (
              <>
            ...
            {activeKey === 'task-trace' && <TaskTraceConsole />}

            {activeKey === 'system-logs' && <SystemLogsPage />}
          </>
        )}
      />
    </div>

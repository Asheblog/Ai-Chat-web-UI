...
export function PersonalModelsPage(){
  const { personalModels, fetchPersonalModels, createPersonalModel, updatePersonalModel, deletePersonalModel, isLoading } = useSettingsStore()
  const { toast } = useToast()
  const [isCreateOpen, setCreateOpen] = useState(false)
  const [editingModel, setEditingModel] = useState<ModelConfig|null>(null)
  const [formData, setFormData] = useState({name:'', apiUrl:'', apiKey:'', supportsImages:false})

  useEffect(()=>{ fetchPersonalModels() },[fetchPersonalModels])
  ...
  const handleUpdate = async()=>{
    ...
    await updatePersonalModel(editingModel.id, {
      name: formData.name,
        ...
        <Dialog open={isCreateOpen} onOpenChange={setCreateOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>配置您的自定义 AI 模型 API</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">开启后聊天框可上传图片传给该模型</p>
                </div>
                ...
                {editingModel?.id === model.id ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="icon" variant="outline" onClick={()=>deletePersonalModel(model.id)} className="text-destructive hover:text-destructive"><Trash2 className="h-4 w-4"/></Button>
                    </div>
                  </>
                )}
              </div>

...
- `model-selector-search-controls.tsx`：搜索框与单行筛选项区域。
- `model-selector-quick-grid.tsx`：常用模型快捷文本项区域。
- `model-selector-group-list.tsx`：桌面端文字系列导航 + 模型列表，移动端系列标签 + 触控模型列表。
...
- 模型识别以文字为主，不引入供应商 Logo / 模型家族图标；图标仅用于搜索、展开、选中、收藏和能力提示。

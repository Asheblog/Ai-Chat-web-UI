...
import { handleRouteError } from '../http/route-utils'
...
export const createPromptTemplatesApi = (deps: PromptTemplatesApiDeps) => {
  ...
  router.get('/', actorMiddleware, requireUserActor, async (c) => {
    try {
      ...
      return handleRouteError(c, error, 'Failed to fetch prompt templates')
    }
  })
  ...
  router.post(
    ...
    async (c) => {
      try {
        ...
        return handleRouteError(c, error, 'Failed to create prompt template')
      }
    },
  )
  ...
  router.put(
    ...
    async (c) => {
      try {
        ...
        return handleRouteError(c, error, 'Failed to update prompt template')
      }
    },
  )
  ...
  router.delete('/:id', actorMiddleware, requireUserActor, async (c) => {
    try {
      ...
      return handleRouteError(c, error, 'Failed to delete prompt template')
    }
  })

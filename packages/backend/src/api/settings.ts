...
export const createSettingsApi = (deps: SettingsApiDeps = {}) => {
  ...
  const systemSettingSchema = z.object({
    ...
    web_search_enabled_engines: z.array(z.enum(['tavily', 'brave', 'metaso'])).min(1).max(3).optional(),
    web_search_engine_order: z.array(z.enum(['tavily', 'brave', 'metaso'])).min(1).max(3).optional(),

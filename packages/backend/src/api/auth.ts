...
export const createAuthApi = (deps: AuthApiDeps = {}) => {
  ...
  router.post('/register', zValidator('json', registerSchema), async (c) => {
    try {
      ...
      const result = await svc.register({ username, password });
  ...
  router.post('/login', zValidator('json', loginSchema), async (c) => {
    try {
      const { username, password } = c.req.valid('json');
      const result = await svc.login({ username, password, request: c.req.raw });
  ...
  router.get('/actor', actorMiddleware, async (c) => {
    ...
    try {
      const context = await svc.resolveActorContext(actor, c.req.raw);
  ...
  // 获取当前用户信息
  router.get('/me', actorMiddleware, requireUserActor, async (c) => {
  ...
  // 更新用户密码
  router.put('/password', actorMiddleware, requireUserActor, zValidator('json', z.object({
    ...
    try {
      ...
      const result = await svc.updatePassword({
        userId: user.id,
        currentPassword,
        newPassword,
      });

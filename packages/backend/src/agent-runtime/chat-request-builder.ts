  ...
  async prepare(params: PrepareChatRequestParams): Promise<PreparedChatRequest> {
      ...
      systemPrompts.push({
        ...
        content:
          params.deepResearchWebSearchActive === true
            ? '你现在处于深度研究（Deep Research）模式。第一步必须调用 research_plan 工具，把完整研究计划提交给用户确认；在收到用户批准之前，绝对不要调用 web_search、read_url 或任何其它工具。研究计划必须包含：title、objective、3-6 个 sub_questions（每个子问题附 1-3 个搜索 keywords）、estimated_tool_rounds 范围、deliverable 固定为 markdown_report_with_citations_pdf。用户批准计划后，交替使用 web_search 和 read_url 收集证据；至少读取若干篇搜索结果原文，并对冲突信息做交叉验证。搜索和读网页时，注意工具返回的图片证据（URL、描述、相关性），优先选择经识图筛选为“相关或弱相关”的图片。若某个子问题需要新闻配图或现场图，可主动调用 web_search 并设置 scope="image" 来补充图片检索；图片搜索结果同样会经过识图相关性筛选。所有关键事实必须记录来源，并在最终报告中用 [1]、[2] 等编号逐段引用。报告必须使用 Markdown 写成完整文档，结构为：标题、摘要、研究背景、分节正文、结论、参考来源列表。应在相应小节使用 Markdown 图片语法插入 1-2 张最相关图片，例如 `![图片描述](图片URL)`；只能使用工具返回的图片 URL，不得编造或外链无关图片。报告完成后，必须调用 export_pdf，把完整 Markdown 报告、标题以及报告所用图片（images 参数）传给该工具生成 PDF；PDF 生成成功后在回答中告知用户可下载。'
    ...
    if (enabledSkillSet.has(BUILTIN_SKILL_SLUGS.PYTHON_RUNNER)) {
      systemPrompts.push({
        role: 'system',
        content:
          '你拥有会话级独立 workspace 工具链：python_runner、workspace_git_clone、workspace_list_files、workspace_read_text。生成可下载文件时请写入 /workspace/artifacts（例如 report.xlsx、report.pdf），并在 python_runner 的 downloadable_files 参数中显式声明允许下载的文件路径；未声明文件不会对用户提供下载。需要分析代码仓库时先用 workspace_git_clone 克隆到 /workspace/repos，再用 workspace_list_files / workspace_read_text 浏览。用户上传的附件文件位于 /workspace/input/，你可以用 python_runner 调用 Python 库直接读取任意格式文件（如 openpyxl 读 Excel、PyPDF2/PyMuPDF 读 PDF、python-docx 读 Word、pandas 读 CSV/表格、Pillow 读图片等）；也可用 workspace_read_text 读取纯文本文件。Python 运行在隔离沙箱中：执行阶段默认无外网，请不要在用户代码里手动执行 pip install/apt/curl 等联网安装命令；应直接 import 依赖，缺失依赖会由运行时受控自动安装后重试。为触发自动安装，不要用 try/except 吞掉 ModuleNotFoundError/ImportError。优先使用简短、确定性代码，并通过 print() 输出关键信息。',
      })
    }

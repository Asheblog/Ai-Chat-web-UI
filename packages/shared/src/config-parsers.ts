/**
 * 通用配置解析原语 —— backend / frontend 共用（RN 安全，无 DOM / Node API）。
 *
 * 收敛 backend utils/parsers.ts 与 frontend 设置页/设置 API 中的重复解析逻辑：
 * - parseBooleanSetting：true/false、1/0、yes/no、y/n、on/off
 * - parseNumberSetting / parseFloatSetting：数值 + 范围限制
 * - parseEnumSetting：枚举归一化
 * - parseDomainListSetting：JSON 数组或逗号分隔
 * - clampNumber / truncateText
 */

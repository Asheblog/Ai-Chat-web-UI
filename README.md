...
- 初始化完成后，前端会进入“初始化向导”（需管理员登录）：按步骤配置 **系统连接** 与 **可用模型** 后才能进入主界面；也可选择“跳过”（全站生效，会提示风险，建议尽快补齐配置）。
...
首次启动引导
- 新库/首次启动会自动进入“初始化向导”（/main 首屏遮罩）：必须使用管理员账号登录后才能继续配置。
- 若需要反复验证向导流程：可清空本地数据库文件，或在启动时临时设置 `DB_INIT_ON_START=true`（仅用于触发引导；请勿长期保留在生产环境）。
